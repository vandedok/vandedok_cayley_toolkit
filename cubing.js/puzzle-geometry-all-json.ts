...
if (globalThis.process && process.argv && process.argv.length >= 3) {
  ...
  const last_argp_before_some_madness = argp;
  const results_json_dict: Record<string, string> = {};
  ...
  if (showargs) {
    if (pascalcomment) {
      ...
      results_json_dict[name] = `(* ${process.argv.join(" ")} *)\n`
    } else {
      // console.log(`# ${process.argv.join(" ")}`);
      results_json_dict[name] = `# ${process.argv.join(" ")}\n`
    }
  ...
  if (dogap) {
    results_json_dict[name] += pg.writegap();
  } else if (domathematica) {
    results_json_dict[name] += pg.writemathematica()
  } else if (doksolve) {
    results_json_dict[name] += pg.writeksolve()
  } else if (dosvg) {
    results_json_dict[name] += pg.generatesvg(undefined, undefined, undefined, do3d)
  } else if (do3d) {
    results_json_dict[name] += String(pg.get3d());
  } else if (doss) {
    throw new Error("SchreierSims (--ss option) is not supported.");
    // const logLines: string[] = [];
    // pg.writeSchreierSims((line) => {logLines.push(line);});
    // results_json_dict[name] = logLines;

  } else if (docanon) {
    throw new Error("Cannon (--cannon option) is not supported.");
      // const canonLines: string[] = [];
      // pg.showcanon((line) => {canonLines.push(line);});
      // results_json_dict[name] = canonLines;
  }
}

const jsonString = JSON.stringify(results_json_dict, null, 2);
// console.log(last_argp_before_some_madness)
// console.log(String(process.argv[last_argp_before_some_madness]))

fs.writeFileSync(process.argv[last_argp_before_some_madness], jsonString, "utf-8");
}
