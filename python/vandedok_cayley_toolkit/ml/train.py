...
def train_with_bellman(cfg: CayleyMLCfg, graph: CayleyGraph, model: torch.nn.Module):
    ...
    for bellman_i in range(cfg.train.bellman.n_updates):
        ...
        y_bellman = bellman_update(
            model, X, y, generators, cfg.train.bellman.bellman_batch_size, graph.central_state
        ...
        for epoch_i in range(cfg.train.bellman.epochs_per_update):
            avg_train_loss, avg_val_loss = regress_epoch_train(
                X_train, X_val, y_train, y_val, model, loss_fn, optimizer, epoch_i, cfg.train.bellman.reg_batch_size
            )
            logger.info(f"Epoch {epoch_i} | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}")
