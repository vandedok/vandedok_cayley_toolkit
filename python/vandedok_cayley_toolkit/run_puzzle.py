...
from .graphs_utils import kamada_kawai_layered_layout, draw_graph_with_nx
...
def run_puzzle_analysis(
    ...
    bfs_result = graph.bfs(
        ...
        max_diameter=max_draw_graph_diameter,
    )
    ...
    graph_max_layers = list(range(1, max_draw_graph_diameter + 1))

    layout_dir = graphs_dir / f"{layout}"
    layout_dir.mkdir(exist_ok=True)
    layers_weights = [10, 10, 70, 70]
    layers_weights = layers_weights[: max_draw_graph_diameter + 1]

    for max_layer, layers_weight in zip(graph_max_layers, layers_weights):
